...
public:
    ...
    bool add(const std::string& name, const std::vector<EigenVectorType>& data, int interleave = 1, int buffer_capacity = -1){
        ...
        return add(name, tmp, interleave, buffer_capacity);
    ...
    void valid_name_transformer( std::string& var_name ) {
        
        bool variable_changed = false;
        // Check that the first character is a letter 
        while ( std::regex_search( var_name, _first_letter_valid_mat ) ) { 
            
            variable_changed = true;
            
            if ( var_name.size() ) {
                var_name = var_name.substr(1, var_name.size());
            }
            // if all the character are not allowed
            else {
                std::chrono::system_clock::time_point p = std::chrono::system_clock::now();
                std::time_t t = std::chrono::system_clock::to_time_t(p);
                var_name = "full_not_valid_" + std::string(std::ctime(&t));
            }
            
        }
        
        // check for not permitted characters and substitute them with _
        var_name = std::string(std::regex_replace( var_name, _not_permitted_char_mat, "_" ));
        
        if( variable_changed ) {
            Logger::info(Logger::Severity::LOW) << "Because of invalid starting characters, variable name changed to :  " << var_name << Logger::endl();
        }
    }
    ...
    void flush(){
        ...
        const char * disable_log = std::getenv("MATLOGGER_DISABLE_LOG");
        if(disable_log && std::atoi(disable_log) != 0)
        {
            Logger::info(Logger::Severity::HIGH, 
                         "Will not flush '%s' as MATLOGGER_DISABLE_LOG is defined and not zero\n", _file_name.c_str());
            return;
        }
        ...
        for( auto& pair : _single_var_map ){
            ...
            std::string valid_name = pair.first;
            valid_name_transformer(valid_name);

            matvar_t * mat_var = Mat_VarCreate(valid_name.c_str(),
                                               MAT_C_DOUBLE,
                                               MAT_T_DOUBLE,
                                               n_dims,
                                               dims,
...
protected:
    ...
    MatLogger(std::string file_name):
        _flushed(false),
        _first_letter_valid_mat("^[^a-zA-Z]"),
        _not_permitted_char_mat("[^a-zA-Z_0-9]")
    {
        // retrieve time
        time_t rawtime;
        struct tm * timeinfo;
...
private:
    ...
    std::regex _first_letter_valid_mat, _not_permitted_char_mat;
