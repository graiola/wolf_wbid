...
private:
    ...
    std::string urdf_string, srdf_string, joint_map_string;
    ...
    void parseJointMap(const YAML::Node& joint_map);
...
public:
     ...
     * @brief initialization function for the model: it loads the URDF and parses the SRDF
     * 
     * @param urdf URDF as string
     * @param srdf SRDF as string
     * @param joint_map_config joint ID map as string
     * @return bool true on success, false otherwise
     */
    bool init_from_string(const std::string& urdf, const std::string& srdf,
              const std::string& joint_map_config);
