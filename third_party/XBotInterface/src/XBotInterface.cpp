...
bool XBot::XBotInterface::init(const XBot::ConfigOptions& options)
{

    _cfg = options;
    
    _path_to_cfg = _cfg.get_path_to_config();
    
    // initialize the model
    if (!_XBotModel.init_from_string(options.get_urdf(), options.get_srdf(), options.get_joint_id_map())) {
    ...
    // save paths (if defined)
    _cfg.get_urdf_path(_urdf_path);
    _cfg.get_srdf_path(_srdf_path);
    ...
    bool success = init_internal(options);
...
{
    return init(ConfigOptions::FromConfigFile(path_to_cfg));
}
...
const std::string& XBot::XBotInterface::getUrdfPath() const
{
    if(_urdf_path == "")
    {
        Logger::warning("in %s: path to URDF is undefined\n");
    }
...
const std::string& XBot::XBotInterface::getSrdfPath() const
{
    if(_srdf_path == "")
    {
        Logger::warning("in %s: path to SRDF is undefined\n");
    }
