...
// calls kdl parser after converting all floating joints to fixed
// on a local urdf copy
bool treeFromUrdfModelNoFloating(const urdf::ModelInterface& urdfmdl,
                                 KDL::Tree& tree)
{
    // make a local copy
    // note: this will be a shallow copy unfortunately
    urdf::ModelInterface urdfmdl_nofloating = urdfmdl;


    // convert all floating to fixed to avoid kdl's warning
    std::vector<std::string> mod_joints_names;
    for(auto item : urdfmdl_nofloating.joints_)
    {
        if(item.second->type == urdf::Joint::FLOATING)
        {
            item.second->type = urdf::Joint::FIXED;
            mod_joints_names.push_back(item.first);
        }
    }

    // parse tree
    bool ret = kdl_parser::treeFromUrdfModel(urdfmdl_nofloating,
                                             tree);

    // note: due to shallow copy, need to restore joints
    // as they were before
    for(auto jname : mod_joints_names)
    {
        urdfmdl_nofloating.joints_.at(jname)->type = urdf::Joint::FLOATING;
    }
...
urdf::ModelInterfaceSharedPtr XBot::XBotCoreModel::loadURDF(const std::string& filename)
{
...
bool XBot::XBotCoreModel::parseSRDF() {
    ...
    for(int i = 0; i < group_num - 1; i++) {
        ...
        if(std::find(chain_names.begin(), chain_names.end(), actual_groups[i].name_) != chain_names.end() ) {
            ...
            /* Check that all joints have a corresponding ID if the joint2rid is not empty!*/
            if(!joint2rid.empty())
            {
                for(auto jname : enabled_joints_in_chains[actual_groups[i].name_])
                {
                    if(joint2rid.find(jname) == joint2rid.end())
                    {
                        throw std::runtime_error("Joint " + jname + " missing in joint ID map");
                    }
                }
            }
...
void XBot::XBotCoreModel::parseJointMap(const YAML::Node& joint_map)
{
                                           ...
                                           const std::string& joint_map_config)
{
    ...
    /* Get urdf dom */
    urdf_model = urdf::parseURDF(urdf);
    
    /* Check joint limits consistency */
    if(!check_joint_limits())
    {
        throw std::runtime_error("Urdf error: joint limits must be specified on all joints");
    }
    
    /* Parse and check joint id map consistency */
    YAML::Node joint_map = YAML::Load(joint_map_config)["joint_map"];
    if(joint_map_config.empty())
        throw std::invalid_argument("joint_map not valid!");
    parseJointMap(joint_map);
    
    
    /* create the robot KDL tree from the URDF model */
    if( !treeFromUrdfModelNoFloating(*urdf_model, robot_tree) ) {
    ...
    bool parse_srdf_success = initString(*urdf_model, srdf);
    std::clog.rdbuf(buf);
    ...
    return parseSRDF() && parse_srdf_success;
}
                               ...
                               const std::string& joint_map_config)
{
    ...
    // parse the Joint map
    YAML::Node joint_map = YAML::LoadFile(joint_map_config)["joint_map"];
    parseJointMap(joint_map);
