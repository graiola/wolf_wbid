...
namespace tf {
inline void vectorKDLToEigen(const KDL::Vector& kdl_vector, Eigen::Vector3d& eigen_vector) {
    eigen_vector.x() = kdl_vector.x();
    eigen_vector.y() = kdl_vector.y();
    eigen_vector.z() = kdl_vector.z();
}

inline void vectorEigenToKDL(const Eigen::Vector3d& eigen_vector, KDL::Vector& kdl_vector) {
    kdl_vector.x(eigen_vector.x());
    kdl_vector.y(eigen_vector.y());
    kdl_vector.z(eigen_vector.z());
}

inline void twistKDLToEigen(const KDL::Twist& kdl_twist, Eigen::Matrix<double, 6, 1>& eigen_twist) {
    eigen_twist(0) = kdl_twist.vel.x();
    eigen_twist(1) = kdl_twist.vel.y();
    eigen_twist(2) = kdl_twist.vel.z();
    eigen_twist(3) = kdl_twist.rot.x();
    eigen_twist(4) = kdl_twist.rot.y();
    eigen_twist(5) = kdl_twist.rot.z();
}

inline void twistEigenToKDL(const Eigen::Matrix<double, 6, 1>& eigen_twist, KDL::Twist& kdl_twist) {
    kdl_twist.vel.x(eigen_twist(0));
    kdl_twist.vel.y(eigen_twist(1));
    kdl_twist.vel.z(eigen_twist(2));
    kdl_twist.rot.x(eigen_twist(3));
    kdl_twist.rot.y(eigen_twist(4));
    kdl_twist.rot.z(eigen_twist(5));
}

inline void transformKDLToEigen(const KDL::Frame& kdl_frame, Eigen::Affine3d& eigen_transform) {
    // Translation
    eigen_transform.translation().x() = kdl_frame.p.x();
    eigen_transform.translation().y() = kdl_frame.p.y();
    eigen_transform.translation().z() = kdl_frame.p.z();

    // Rotation
    eigen_transform.linear()(0, 0) = kdl_frame.M(0, 0);
    eigen_transform.linear()(0, 1) = kdl_frame.M(0, 1);
    eigen_transform.linear()(0, 2) = kdl_frame.M(0, 2);
    eigen_transform.linear()(1, 0) = kdl_frame.M(1, 0);
    eigen_transform.linear()(1, 1) = kdl_frame.M(1, 1);
    eigen_transform.linear()(1, 2) = kdl_frame.M(1, 2);
    eigen_transform.linear()(2, 0) = kdl_frame.M(2, 0);
    eigen_transform.linear()(2, 1) = kdl_frame.M(2, 1);
    eigen_transform.linear()(2, 2) = kdl_frame.M(2, 2);
}

inline void transformEigenToKDL(const Eigen::Affine3d& eigen_transform, KDL::Frame& kdl_frame) {
   // Translation
   kdl_frame.p.x(eigen_transform.translation().x());
   kdl_frame.p.y(eigen_transform.translation().y());
   kdl_frame.p.z(eigen_transform.translation().z());

   // Rotation
   kdl_frame.M = KDL::Rotation(
       eigen_transform.linear()(0, 0), eigen_transform.linear()(0, 1), eigen_transform.linear()(0, 2),
       eigen_transform.linear()(1, 0), eigen_transform.linear()(1, 1), eigen_transform.linear()(1, 2),
       eigen_transform.linear()(2, 0), eigen_transform.linear()(2, 1), eigen_transform.linear()(2, 2)
   );
}
}
...
public:
    ...
    * @param offset Rotation are premultiplied by this roation offset
    ...
    */
    ...
    /**
    * @brief Gets the velocity twist of link_name (first linear, then angular velocity) 
    * w.r.t. a user-specified base-link.
    * The reference point of the velocity twist is the origin of the link itself.
    *
    * @param link_name The link for which the twist is computed
    * @param base_link_name The base link for the computed velocity
    * @param velocity The twist of the link "link_name" w.r.t. "base_link_name"
    * @return True if the link_name and base_link_name are valid link names. False otherwise.
    */
    bool getRelativeVelocityTwist( const std::string& link_name, const std::string& base_link_name,
                             KDL::Twist& velocity) const;

    /**
    * @brief Computes the velocity of the source_frame w.r.t. the target_frame expressed in target_frame
    *
    * @param source_frame The source link name.
    * @param target_frame The target link name.
    * @param velocity A twist computed as:
    *   v = J*qdot,
    * where J is the relative Jacobian computed using getRelativeJacobian(source_frame, target_frame)
    * @return True if both source_frame and target_frame are valid. False otherwise.
    */
    ...
    */
    ...
    /**
     * @brief getRelativeAccelerationTwist compute the relative acceleration of body "link" wrt body "base_link"
     * expressed in frame "base_link"
     * @param link_name
     * @param base_link_name
     * @param acceleration relative acceleration
     * @return true if link and base_link names are valid. False otherwise
     */
    ...
    */
    ...
    /**
     * @brief computeRelativeJdotQdot return the relative acceleration component between "target_link" and
     * "base_link", given by JdotQdot, expressed in "base_link"
     * @param target_link_name
     * @param base_link_name
     * @param jdotqdot
     * @return true if target_link name and base_link name are valid. False otherwise
     */
    ...
    /**
     * @brief Computes the non-linear torque term without the contribution from gravity,
     * i.e. the sum of centrifugal/coriolis torques.
     *
