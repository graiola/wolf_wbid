...
XBot::ModelInterface::Ptr XBot::ModelInterface::getModel(const XBot::ConfigOptions &config)
{
    /* Check model floating base */
    bool is_model_floating_base;
    if (!config.get_parameter("is_model_floating_base", is_model_floating_base)) {
        throw std::invalid_argument("Provided configuration object must contain a bool-typed \"is_model_floating_base\" field");
    }

    /* Obtain model type */
    std::string model_type;
    if (!config.get_parameter("model_type", model_type)) {
        throw std::invalid_argument("Provided configuration object must contain a std::string-typed \"model_type\" field");
    }


    /* Obtain full path to shared lib */
    std::string path_to_shared_lib = XBot::Utils::FindLib("libModelInterface" + model_type + ".so", "LD_LIBRARY_PATH");
    if (path_to_shared_lib == "") {
        throw std::runtime_error("libModelInterface" + model_type + ".so path must be listed inside LD_LIBRARY_PATH");
    }

    auto instance_ptr  = SoLib::getFactory<XBot::ModelInterface>(path_to_shared_lib, "MODEL INTERFACE");
    if (instance_ptr) {
...
XBot::ModelInterface::Ptr XBot::ModelInterface::getModel(const std::string &path_to_config, AnyMapConstPtr any_map)
{
    ...
    return getModel(ConfigOptions::FromConfigFile(abs_path_to_cfg));
}
...
bool XBot::ModelInterface::init_internal(const ConfigOptions &config)
{
    ...
    _compute_inverse_inertia = true;
    return true;
}



bool XBot::ModelInterface::update(bool update_position, bool update_velocity, bool update_desired_acceleration)
{
    _compute_inverse_inertia = true;
    return update_internal(update_position, update_velocity, update_desired_acceleration);
}
        ...
        KDL::Jacobian &J) const
{
    ...
    base_jacobian.changeRefPoint(world_origin_target - world_origin_base);
                                                    ...
                                                    KDL::Twist& velocity) const
{
    ...
    /* NOTE INEFFICIENT TODO FIX THE BETTER ONE */
    
    if(!getRelativeJacobian(link_name, base_link_name, _tmp_kdl_jacobian))
    {
        return false;
    }
    
    getJointVelocity(_tmp_jstate);
    Eigen::Vector6d velocity_eig;
    velocity_eig.noalias() = _tmp_kdl_jacobian.data * _tmp_jstate;
    
    tf::twistEigenToKDL(velocity_eig, velocity);
...
{
    KDL::Twist v_rel;
    if(!getRelativeVelocityTwist(link_name, base_link_name, v_rel))
    {
        return false;
    }
    
    tf::twistKDLToEigen(v_rel, velocity);
    return true;
}

bool XBot::ModelInterface::getRelativeAccelerationTwist(const std::string& link_name, const std::string& base_link_name,
                                          Eigen::Vector6d& acceleration) const
{
    KDL::Twist a_rel;
    if(!getRelativeAccelerationTwist(link_name, base_link_name, a_rel))
    {
        return false;
    }

    tf::twistKDLToEigen(a_rel, acceleration);
    return true;
}
...
{
    bool ret = true;
    ret = getPose(source_frame, _tmp_kdl_frame) && ret;
    ret = getPose(target_frame, _tmp_kdl_frame_1) && ret;
                               ...
                               KDL::Twist& velocity) const
{
    ...
    bool success = getVelocityTwist(source_frame, target_frame, eigen_vec6d);
    tf::twistEigenToKDL(eigen_vec6d, velocity);
    return success;
}
...
{
    bool success = false;
    Eigen::Vector3d _tmp_point;
    tf::vectorKDLToEigen(point, _tmp_point);
    success = getJacobian(link_name, _tmp_point, _tmp_jacobian);
    getJointVelocity(_tmp_jstate);
    ::computeJdot(_tmp_jacobian, _tmp_jstate, _tmp_jacobiandot);
    _tmp_jdot_qdot.noalias() = _tmp_jacobiandot * _tmp_jstate;

    tf::twistEigenToKDL(_tmp_jdot_qdot, jdotqdot);
...
{
    bool success = computeRelativeJdotQdot(target_link_name, base_link_name, _tmp_kdl_twist);
    tf::twistKDLToEigen(_tmp_kdl_twist, jdotqdot);
    return success;
}
        ...
        Eigen::Vector6d &CMMdotQdot) const
{
    ...
    centroidal_momentum_matrix.noalias() = Ju_T_inv * _tmp_inertia.block(0, 0, 6, getJointNum());
    CMMdotQdot.noalias() = Ju_T_inv * (_tmp_h - _tmp_gcomp).head<6>();
...
void XBot::ModelInterface::getInertiaInverseTimesMatrix(const Eigen::MatrixXd &Mat, Eigen::MatrixXd &Minv_Mat) const
{
    ...
    _tmp_inv_inertia_vec.setZero(getJointNum());

    for (int i = 0; i < Minv_Mat.rows(); ++i) {
        _M_col = Mat.col(i);
        getInertiaInverseTimesVector(_M_col, _tmp_inv_inertia_vec);
        Minv_Mat.col(i) = _tmp_inv_inertia_vec;
    }
...
{
    if (_compute_inverse_inertia) {
        _tmp_I.setIdentity(getJointNum(), getJointNum());
        getInertiaInverseTimesMatrix(_tmp_I, _tmp_inverse_inertia);
        _compute_inverse_inertia = false;
    }

    Minv = _tmp_inverse_inertia;
}
...
bool XBot::ModelInterface::setFloatingBaseState(XBot::ImuSensor::ConstPtr imu, const Eigen::Matrix3d& offset)
{
    Eigen::Matrix3d fb_R_imu;
    getFloatingBaseLink(_floating_base_link);
    if(!getOrientation(imu->getSensorName(), _floating_base_link,  fb_R_imu))
        return false;
    ...
    Eigen::Matrix3d ned_R_imu;
    Eigen::Vector3d imu_ang_vel;
    imu->getOrientation(ned_R_imu);
    imu->getAngularVelocity(imu_ang_vel);

    Eigen::Matrix3d w_R_imu = offset*ned_R_imu;
